...
public abstract class CircuitElm implements Editable {
    ...
    double volts[], voltsi[];
    double current, currenti, curcount;
...
//	noCommaFormat.setGroupingUsed(false);
    ...
    // easy way to get/set capacitance, resistance, etc. without casts
    double getValue() { return 0; }
    void setValue(double q) { }

    void draw(Graphics g) {}
    void setCurrent(int x, double cr, double ci) {
        current  = cr;
        currenti = ci;
    }
    ...
    void setNodeVoltage(int n, double cr, double ci) {
        volts[n] = cr;
        voltsi[n] = ci;
        calculateCurrent();
    }
...
//	int ypd = b.y-a.y;
    ...
    void drawDots(Graphics g, Point pa, Point pb, double pos) {
	 if (sim.stoppedCheck.getState() || sim.animateFreq <= 0 ||
		 pos == 0 || !sim.dotsCheckItem.getState())
	    return;
	int dx = pb.x-pa.x;
	int dy = pb.y-pa.y;
...
//		   x+w, y+fm.getAscent()/2+fm.getDescent());
	...
	// compose with current circuit transform rather than replacing it
...
/*	int i;
    ...
    static String getPeakVText(double vr, double vi) {
        return getPeakText(vr, vi, "V");
    }
    static String getPeakIText(double vr, double vi) {
        return getPeakText(vr, vi, "A");
    }
    static String getPeakText(double vr, double vi, String unit) {
        double mag = Math.sqrt(vr*vr+vi*vi);
        double phase = (Math.atan2(vi, vr)-sim.phaseShift.phase())*180/Math.PI;
        while (phase < -180)
            phase += 360;
        while (phase >= 180)
            phase -= 360;
        return "peak " + getUnitText(mag, unit) + ", " +
            showFormat.format(phase) + "\u00b0";
    }
    ...
    int getBasicInfo(String arr[]) {
        if (sim.animateFreq <= 0)
            return 1;
        arr[1] = "I = " + getCurrentDText(getCurrent());
        arr[2] = getPeakIText(current, currenti);
        arr[3] = "Vd = " + getVoltageDText(getVoltageDiff());
        arr[4] = getPeakVText(volts [0]-volts [1],
                              voltsi[0]-voltsi[1]);
        return 5;
    }
    ...
    Color getVoltageColor(Graphics g, double volts) {
    	...
    	if (!sim.voltsCheckItem.getState()) {
    	    ...
    	    !sim.admittanceCheckItem.getState())
    	    	return(whiteColor);
    	    return (g.lastColor);
    	}
    ...
    void setAdmittanceColor(Graphics g, double r) {
        if (!sim.admittanceCheckItem.getState())
            return;
        if (isSelected() || sim.selectedFreq <= 0)
            return;
        int rg = (int) (10000/r);
        int dark = 64;
        int lite = 256-dark;
        if (rg >= lite) {
            ...
            if (rg > 255)
                rg = 255;
            g.setColor(new Color(255, 255, rg));
            return;
        }
        rg += dark;
        g.setColor(new Color(rg, rg, dark));
    }
