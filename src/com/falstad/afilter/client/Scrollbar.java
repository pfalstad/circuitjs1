...
public class Scrollbar extends  Composite implements 
	...
	Customizer attachedCustomizer=null;
	...
	public Scrollbar(int orientation, int value, int visible, int minimum, int maximum, 
		Customizer cmd, Customizer e) {
	this(orientation,value,visible,minimum,maximum);
	attachedCustomizer=e;
}
...
//	}
	...
	void setValues(int v, int vis, int mn, int mx) {
	    min = mn;
	    max = mx-vis;
	    val = v;
	    CirSim.console("setvalues " + v + " " + vis + " " + mn + " " + mx);
	    draw();
	}
...
//		GWT.log("Down");
		...
		if (enabled){
			...
			if (command!=null)
				command.execute();
			if (attachedCustomizer != null)
			    attachedCustomizer.slidersChanged(this);
		}
