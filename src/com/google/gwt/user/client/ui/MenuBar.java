  ...
  private final class MenuPopup extends DecoratedPopupPanel {
    ...
    protected void onPreviewNativeEvent(NativePreviewEvent event) {
      ...
      if (!event.isCanceled()) {
        ...
        // On modern touch browsers (e.g. Safari/iPad, especially with Apple
        // Pencil), a pointerdown/touchstart fires before the compatibility
        // mousedown event. Without suppressing those events too, a tap on a
        // sibling submenu inside the parent menu closes the whole popup
        // hierarchy before the mousedown-based protection below ever runs.
        String nativeType = event.getNativeEvent().getType();
        if ("pointerdown".equals(nativeType) || "touchstart".equals(nativeType)) {
          EventTarget target = event.getNativeEvent().getEventTarget();
          Element parentMenuElement = MenuBar.this.getElement();
          if (Element.is(target) && parentMenuElement.isOrHasChild(Element.as(target))) {
            event.cancel();
            return;
          }
        }
