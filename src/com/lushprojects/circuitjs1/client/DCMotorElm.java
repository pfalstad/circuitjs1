...
class DCMotorElm extends CircuitElm {
    ...
    public DCMotorElm(int xa, int ya, int xb, int yb, int f, StringTokenizer st) {
	super(xa, ya, xb, yb, f);
	angle = pi / 2;
	speed = 0;
	// read:
	...
	K = Double.valueOf(st.nextToken()).doubleValue();
	Kb = Double.valueOf(st.nextToken()).doubleValue();
	J = Double.valueOf(st.nextToken()).doubleValue();
	b = Double.valueOf(st.nextToken()).doubleValue();
	gearRatio = Double.valueOf(st.nextToken()).doubleValue();
	tau = Double.valueOf(st.nextToken()).doubleValue();
	...
	return super.dump() + " " + inductance + " " + resistance + " " + K + " " + Kb + " " + J + " " + b + " "
		+ gearRatio + " " + tau;
    }
    ...
    /*
     * boolean hasGroundConnection(int n1) { if (n1==4|n1==5) return true; else
     * return false; } boolean getConnection(int n1, int n2) {
     * if((n1==0&n2==2)|(n1==2&n2==3)|(n1==1&n2==3)|(n1==4&n2==5)) return true; else
     * return false; }
     */
    ...
    // public double getCurrent() { current = (volts[2]-volts[3])/resistance; return
    // current; }
	...
	g.setColor(cc);
	interpPointFix(lead1, lead2, ps1, 0.5 + .28 * Math.cos(angleAux * gearRatio),
		.28 * Math.sin(angleAux * gearRatio));
	interpPointFix(lead1, lead2, ps2, 0.5 - .28 * Math.cos(angleAux * gearRatio),
		-.28 * Math.sin(angleAux * gearRatio));

	drawThickerLine(g, ps1, ps2);
	interpPointFix(lead1, lead2, ps1, 0.5 + .28 * Math.cos(angleAux * gearRatio + pi / 3),
		.28 * Math.sin(angleAux * gearRatio + pi / 3));
	interpPointFix(lead1, lead2, ps2, 0.5 - .28 * Math.cos(angleAux * gearRatio + pi / 3),
		-.28 * Math.sin(angleAux * gearRatio + pi / 3));

	drawThickerLine(g, ps1, ps2);

	interpPointFix(lead1, lead2, ps1, 0.5 + .28 * Math.cos(angleAux * gearRatio + 2 * pi / 3),
		.28 * Math.sin(angleAux * gearRatio + 2 * pi / 3));
	interpPointFix(lead1, lead2, ps2, 0.5 - .28 * Math.cos(angleAux * gearRatio + 2 * pi / 3),
		-.28 * Math.sin(angleAux * gearRatio + 2 * pi / 3));
