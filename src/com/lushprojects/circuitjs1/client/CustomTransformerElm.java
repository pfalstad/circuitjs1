...
class CustomTransformerElm extends CircuitElm {
    ...
    public CustomTransformerElm(int xa, int ya, int xb, int yb, int f, StringTokenizer st) {
	super(xa, ya, xb, yb, f);
	...
	coilCount = Integer.valueOf(st.nextToken()).intValue();
	int i;
	coilCurrents = new double[coilCount];
	for (i = 0; i != coilCount; i++)
	    coilCurrents[i] = Double.valueOf(st.nextToken()).doubleValue();
	noDiagonal = true;
	parseDescription(description);
    }
    ...
    String dump() {
	String s = super.dump() + " " + inductance + " " + couplingCoef + " " + CustomLogicModel.escape(description)
		+ " " + coilCount + " ";
	int i;
	for (i = 0; i != coilCount; i++) {
	    s += coilCurrents[i] + " ";
	...
	if (needDots) {
	    ...
	    for (i = 0; i != coilCount; i++) {
		int n = coilNodes[i];
		dots[i] = interpPoint(nodeTaps[n], nodeTaps[n + 1], coilPolarities[i] > 0 ? dotp : 1 - dotp,
			i < primaryCoils ? -7 : 7);
	    }
	} else
	    dots = null;
	...
	for (i = 0; i != coilCount; i++)
		...
		xformMatrix[i][j] = xformMatrix[j][i] = couplingCoef
			* Math.sqrt(coilInductances[i] * coilInductances[j]) * coilPolarities[i] * coilPolarities[j];
