...
class TransistorElm extends CircuitElm {
	...
	super(xa, ya, xb, yb, f);
	pnp = Integer.valueOf(st.nextToken()).intValue();
	beta = 100;
	try {
	    lastvbe = Double.valueOf(st.nextToken()).doubleValue();
	    lastvbc = Double.valueOf(st.nextToken()).doubleValue();
	    volts[0] = 0;
	    volts[1] = -lastvbe;
	    volts[2] = -lastvbc;
	    beta = Double.valueOf(st.nextToken()).doubleValue();
	...
	return super.dump() + " " + pnp + " " + (volts[0] - volts[1]) + " " + (volts[0] - volts[2]) + " " + beta + " "
		+ CustomLogicModel.escape(modelName);
    }
	...
	/*
	 * double gx=rbpr+rbpi/qb; // base resistance commented out for now if(xjrb !=
	 * 0) { double arg1=Math.max(cb/xjrb,1e-9); double
	 * arg2=(-1+Math.sqrt(1+14.59025*arg1))/2.4317/Math.sqrt(arg1);
	 * arg1=Math.tan(arg2); gx=rbpr+3*rbpi*(arg1-arg2)/arg2/arg1/arg1; } if(gx != 0)
	 * gx=1/gx;
	 */
	...
	arr[0] = Locale.LS("transistor") + " (" + ((pnp == -1) ? "PNP" : "NPN") + ", " + model.name + ", \u03b2="
