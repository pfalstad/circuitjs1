...
class VCVSElm extends VCCSElm {
    public VCVSElm(int xa, int ya, int xb, int yb, int f, StringTokenizer st) {
	...
	if (expr != null) {
	    ...
	    if (Math.abs(volts[inputCount] - volts[inputCount + 1] - v0) > Math.abs(v0) * .01
		...
		// sim.console("ccedx " + i + " " + dx + " v " + v + " v2 " + v2 + " dv " + dv +
		// " lv " + lastVolts[i] + " " + volts[i] + " " + sim.subIterations + " " +
		// sim.t);
