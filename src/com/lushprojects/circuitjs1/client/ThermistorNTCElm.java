...
class ThermistorNTCElm extends CircuitElm implements Command, MouseWheelHandler {
    ...
    public ThermistorNTCElm(int xa, int ya, int xb, int yb, int f, StringTokenizer st) {
	super(xa, ya, xb, yb, f);
	r25 = Double.valueOf(st.nextToken()).doubleValue();
	r50 = Double.valueOf(st.nextToken()).doubleValue();
	minTempr = Double.valueOf(st.nextToken()).doubleValue();
	maxTempr = Double.valueOf(st.nextToken()).doubleValue();
    ...
    // constructor
    String dump() {
	return super.dump() + " " + r25 + " " + r50 + " " + minTempr + " " + maxTempr + " " + position + " "
	...
	g.context.beginPath(); // thermistor symbol lines 0 is in the middle of the left handside of the
			       // resistor box
