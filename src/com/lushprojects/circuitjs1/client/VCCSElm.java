...
class VCCSElm extends ChipElm {
    ...
    public VCCSElm(int xa, int ya, int xb, int yb, int f, StringTokenizer st) {
	...
	// get maximum change in voltage per step when testing for convergence. be more
	// lenient over time
	...
	for (i = 0; i != inputCount; i++) {
		...
		sim.converged = false;
		// sim.console("vcvs " + nodes + " " + i + " " + volts[i] + " " + lastVolts[i] +
		// " " + sim.subIterations);
	    }
	...
	if (expr != null) {
	    ...
	    // if (Math.abs(volts[inputCount]-v0) > Math.abs(v0)*.01 && sim.subIterations <
	    // 100)
	    // sim.converged = false;
	    double rs = v0;
