...
class InvertingSchmittElm extends CircuitElm {
	...
	try {
	    ...
	    lowerTrigger = Double.valueOf(st.nextToken()).doubleValue();
	    upperTrigger = Double.valueOf(st.nextToken()).doubleValue();
	    logicOnLevel = Double.valueOf(st.nextToken()).doubleValue();
	    logicOffLevel = Double.valueOf(st.nextToken()).doubleValue();
	} catch (Exception e) {
	}
    }

    String dump() {
	return super.dump() + " " + slewRate + " " + lowerTrigger + " " + upperTrigger + " " + logicOnLevel + " "
		+ logicOffLevel;
    }
