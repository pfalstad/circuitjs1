...
class ScopePlot {
	...
	// power can be negative for caps and inductors, but still move to the bottom
...
class Scope {
	...
	if (val == 0) {
	    ...
	    if (ce != null && sim.dotsCheckItem.getState() && !(ce instanceof OutputElm || ce instanceof LogicOutputElm
		    || ce instanceof AudioOutputElm || ce instanceof ProbeElm))
	...
	if (yelm != null) {
	    plots = new Vector<ScopePlot>();
	    plots.add(
		    new ScopePlot(ce, ce.getScopeUnits(val), 0, getManScaleFromMaxScale(ce.getScopeUnits(val), false)));
	...
	/*
	 * // if voltage and current are shown, remove current if (plots.size() == 2 &&
	 * plots.get(0).elm == plots.get(1).elm) plots.remove(1); if (s.plots.size() ==
	 * 2 && s.plots.get(0).elm == s.plots.get(1).elm) plots.add(s.plots.get(0));
	 * else
	 */
	plots = visiblePlots;
	...
	if (plot2d && imageContext != null && plots.size() >= 2) {
		...
		x = (int) (rect.width * .499 + (v / plots.get(0).manScale) * gridPx
			+ gridPx * manDivisions * (double) (plots.get(0).manVPosition) / (double) (V_POSITION_STEPS));
		y = (int) (rect.height * .499 - (yval / plots.get(1).manScale) * gridPx
			- gridPx * manDivisions * (double) (plots.get(1).manVPosition) / (double) (V_POSITION_STEPS));
    ...
    /*
     * void adjustScale(double x) { scale[UNITS_V] *= x; scale[UNITS_A] *= x;
     * scale[UNITS_OHMS] *= x; scale[UNITS_W] *= x; scaleX *= x; scaleY *= x; }
     */
	...
	if (isManualScale()) {
		...
		if (i != 0)
		    g.drawLine((int) (gridPx * i) + rect.width / 2, 0, (int) (gridPx * i) + rect.width / 2,
			    rect.height);
	...
	return showSettingsWheel() && sim.mouseCursorX >= rect.x && sim.mouseCursorX <= rect.x + 36
		&& sim.mouseCursorY >= rect.y + rect.height - 36 && sim.mouseCursorY <= rect.y + rect.height;
    }
	...
	if (drawGridLines) {
	    ...
	    // don't show hgridlines if lines are too close together (except for center
	    // line)
	    boolean showHGridLines = (gridStepY != 0) && (isManualScale() || allPlotsSameUnits); // Will only show
												 // center line if we
												 // have mixed units
	...
	// show FFT even if there's no plots (in which case cursorTime/cursorX will be
	// invalid)
	...
	String t = text;
	if (t == null) {
	    // if we're drawing the info and showElmInfo is true, return null so we don't
	    // print redundant info.
	    // But don't do that if we're getting the scope label to generate "Add to
	    // Existing Scope" menu.
    ...
    int getFlags() {
	int flags = (showI ? 1 : 0) | (showV ? 2 : 0) | (showMax ? 0 : 4) | // showMax used to be always on
		(showFreq ? 8 : 0) |
		// In this version we always dump manual settings using the PERPLOT format
		(isManualScale() ? (FLAG_MAN_SCALE | FLAG_PERPLOT_MAN_SCALE) : 0) | (plot2d ? 64 : 0)
		| (plotXY ? 128 : 0) | (showMin ? 256 : 0) | (showScale ? 512 : 0) | (showFFT ? 1024 : 0)
		| (maxScale ? 8192 : 0) | (showRMS ? 16384 : 0) | (showDutyCycle ? 32768 : 0)
		| (logSpectrum ? 65536 : 0) | (showAverage ? (1 << 17) : 0) | (showElmInfo ? (1 << 20) : 0);
	...
	if (eno < 0)
	    return null;
	String x = "o " + eno + " " + vPlot.scopePlotSpeed + " " + vPlot.value + " "
		+ exportAsDecOrHex(flags, FLAG_PERPLOTFLAGS) + " " + scale[UNITS_V] + " " + scale[UNITS_A] + " "
		+ position + " " + plots.size();
	int i;
	for (i = 0; i < plots.size(); i++) {
	...
	int value = Integer.valueOf(st.nextToken()).intValue();
	...
	scale[UNITS_V] = Double.valueOf(st.nextToken()).doubleValue();
	scale[UNITS_A] = Double.valueOf(st.nextToken()).doubleValue();
	if (scale[UNITS_V] == 0)
	    scale[UNITS_V] = .5;
	if (scale[UNITS_A] == 0)
	    scale[UNITS_A] = 1;
	...
	if ((flags & FLAG_PLOTS) != 0) {
		...
		if ((flags & FLAG_IVALUE) != 0) {
		    ivalue = Integer.valueOf(st.nextToken()).intValue();
		}
		while (st.hasMoreTokens()) {
		    if (text == null)
			text = st.nextToken();
