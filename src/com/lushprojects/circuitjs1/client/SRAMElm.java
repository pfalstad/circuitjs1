...
class SRAMElm extends ChipElm {
    ...
    public SRAMElm(int xa, int ya, int xb, int yb, int f, StringTokenizer st) {
	...
	try {
	    // load contents
	    // format: addr val(addr) val(addr+1) val(addr+2) ... -1 addr val val ... -1 ...
	    // -2
	    while (true) {
	...
	for (i = 0; i != dataBits; i++) {
	    Pin p = pins[i + dataNodes];
	    sim.updateVoltageSource(0, nodes[internalNodes + i], p.voltSource,
		    (data & (1 << (dataBits - 1 - i))) == 0 ? 0 : 5);
