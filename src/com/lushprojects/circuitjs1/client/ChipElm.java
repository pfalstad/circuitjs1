...
abstract class ChipElm extends CircuitElm {
	...
	for (i = 0; i != getPostCount(); i++) {
	    ...
	    // (we could still do this if vertical pins are present but then we would have
	    // to do
	...
	for (i = 0; i != getPostCount(); i++) {
	    Pin p = pins[i];
	    if (p.output)
		sim.updateVoltageSource(0, nodes[i], p.voltSource, p.value ? highVoltage : 0);
	}
    }
	...
	void setPoint(int px, int py, int dx, int dy, int dax, int day, int sx, int sy) {
		...
		if (text.length() > 0) {
		    // See for example
		    // http://127.0.0.1:8000/circuitjs.html?ctz=CQAgjCAMB0l3BWcMBMcUHYMGZIA4UA2ATmIxAUgpABZsKBTAWjDACgAncDQkPKlDSr8oySGzTkwPPlTAo8s2iADCAGQDSALgCSAOQBqWle0khBwgUJDYUy9dv1GVKCZHIXwNGubyKw3vaauobG2G5SMgE+0rxgxHY+DiHONJzcvCKxXj5y8OnZ0ebWRXlw6Z5FniJl4kA
	...
	// convert position, side to a grid position (0=top left) so we can detect
	// overlaps
