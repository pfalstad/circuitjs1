...
abstract class GateElm extends CircuitElm {
	...
	super(xa, ya, xb, yb, f);
	inputCount = Integer.valueOf(st.nextToken()).intValue();
	double lastOutputVoltage = Double.valueOf(st.nextToken()).doubleValue();
	noDiagonal = true;
	highVoltage = 5;
	try {
	    highVoltage = Double.valueOf(st.nextToken()).doubleValue();
	} catch (Exception e) {
	}
