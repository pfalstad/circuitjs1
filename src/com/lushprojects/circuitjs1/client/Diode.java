...
class Diode {
	...
	// sim.console("setup " + leakage + " " + zvoltage + " " +
	// model.emissionCoefficient + " " + vdcoef);

	// critical voltage for limiting; current is vscale/sqrt(2) at
	// this voltage
	vcrit = vscale * Math.log(vscale / (Math.sqrt(2) * leakage));
	// translated, *positive* critical voltage for limiting in Zener breakdown
	// region;
	// limitstep() uses this with translated voltages in an analogous fashion to
	// vcrit.
    ...
    // The diode's "scale voltage", the voltage increase which will raise current by
    // a factor of e.
    double vscale;
    // The multiplicative equivalent of dividing by vscale (for speed).
    double vdcoef;
    // The Zener breakdown curve is represented by a steeper exponential, one like
    // the ideal
    // Shockley curve, but flipped and translated. This curve removes the moderating
    // influence
    ...
    // The diode current's scale factor, calculated from the user-specified forward
    // voltage drop.
    double leakage;
    // Voltage offset for Zener breakdown exponential, calculated from
    // user-specified Zener voltage.
    double zoffset;
    // Critical voltages for limiting the normal diode and Zener breakdown
    // exponentials.
    double vcrit, vzcrit;
    double lastvoltdiff;
	...
	// To prevent a possible singular matrix or other numeric issues, put a tiny
	// conductance
	...
	if (voltdiff >= 0 || zvoltage == 0) {
	    ...
	    double geq = leakage
		    * (vdcoef * Math.exp(voltdiff * vdcoef) + vzcoef * Math.exp((-voltdiff - zoffset) * vzcoef)) + gmin;

	    double nc = leakage * (Math.exp(voltdiff * vdcoef) - Math.exp((-voltdiff - zoffset) * vzcoef) - 1)
		    + geq * (-voltdiff);
	...
	return leakage * (Math.exp(voltdiff * vdcoef) - Math.exp((-voltdiff - zoffset) * vzcoef) - 1);
    }
}
