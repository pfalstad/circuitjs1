...
class CCCSElm extends VCCSElm {
    ...
    public CCCSElm(int xa, int ya, int xb, int yb, int f, StringTokenizer st) {
	...
	if (expr != null) {
		...
		// sim.console("ccedx " + i + " " + cur + " " + dx + " " + rs + " " +
		// sim.subIterations + " " + sim.t);
		setCurrentExprValue(i, cur);
	    }
	...
	arr[i++] = pins[j].text + " = " + getVoltageText(volts[j]) + "; " + pins[j + 1].text + " = "
		+ getVoltageText(volts[j + 1]);
