...
class RelayElm extends CircuitElm {
	...
	try {
	    ...
	    switchingTime = 0;
	    offCurrent = Double.valueOf(st.nextToken()).doubleValue();
	...
	return super.dump() + " " + poleCount + " " + inductance + " " + coilCurrent + " " + r_on + " " + r_off + " "
		+ onCurrent + " " + coilR + " " + offCurrent + " " + switchingTime + " " + i_position;
    }
	...
	for (i = 0; i != poleCount; i++) {
		...
		interpPoint(point1, point2, lines[i * 2], .5, openhs * 2 + 5 * dsign - i * openhs * 3 + off);
	    } else
	...
	arr[ln++] = Locale.LS("coil Vd") + " = " + getVoltageDText(volts[nCoil1] - volts[nCoil2]);
    }
