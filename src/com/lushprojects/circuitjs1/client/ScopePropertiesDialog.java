...
public class ScopePropertiesDialog extends Dialog implements ValueChangeHandler<Boolean> {
	...
	super();
	// We are going to try and keep the panel below the target height (defined to
	// give some space)
	...
	boolean displayScales = allowedHeight > 470; // We can display the scales and any one other section. So expand
						     // scales and collapse rest
	sim = asim;
	scope = s;
	...
	positionBar = new Scrollbar(Scrollbar.HORIZONTAL, 0, 1, -Scope.V_POSITION_STEPS, Scope.V_POSITION_STEPS,
		new Command() {
		    public void execute() {
			positionBarChanged();
		    }
	...
	if (vScaleLabel.expanded) {
	    vScaleGrid.getRowFormatter().setVisible(0,
		    scope.isManualScale() && plotSelection < scope.visiblePlots.size());
	    vScaleGrid.getRowFormatter().setVisible(1,
		    scope.isManualScale() && plotSelection < scope.visiblePlots.size());
	    vScaleGrid.getRowFormatter().setVisible(2,
		    (!scope.isManualScale()) || plotSelection < scope.visiblePlots.size());
	}
    ...
    void refreshDraw() {
	// Redraw for every step of the simulation (the simulation may run in the
	// background of this
