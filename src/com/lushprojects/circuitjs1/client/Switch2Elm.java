...
class Switch2Elm extends SwitchElm {
	...
	throwCount = 2;
	try {
	    throwCount = Integer.valueOf(st.nextToken()).intValue();
	} catch (Exception e) {
	}
	noDiagonal = true;
    }
	...
	/*
	 * if (n == 1) { EditInfo ei = new EditInfo("", 0, -1, -1); ei.checkbox = new
	 * Checkbox("Center Off", hasCenterOff()); return ei; }
	 */
	if (n == 1)
	...
	/*
	 * if (n == 1) { flags &= ~FLAG_CENTER_OFF; if (ei.checkbox.getState()) flags |=
	 * FLAG_CENTER_OFF; if (hasCenterOff()) momentary = false; setPoints(); } else
	 */
	if (n == 1) {
	    link = (int) ei.value;
	} else if (n == 2) {
