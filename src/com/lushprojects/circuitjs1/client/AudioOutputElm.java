...
public class AudioOutputElm extends CircuitElm {
    ...
    public AudioOutputElm(int xa, int ya, int xb, int yb, int f, StringTokenizer st) {
	super(xa, ya, xb, yb, f);
	 ...
	 * sim.timeStep = sampleStep; else { // make sure sampleStep/timeStep is an
	 * integer. otherwise we get distortion // int frac =
	 * (int)Math.round(sampleStep/sim.timeStep); // sim.timeStep = sampleStep /
	 * frac;
	 * 
	 * // actually, just make timestep = 1/sampleRate sim.timeStep = sampleStep; }
	 */

	// int frac = (int)Math.round(Math.max(sampleStep*33000, 1));
	double target = sampleStep / 8;
	if (sim.maxTimeStep != target) {
	    if (okToChangeTimeStep
		    || Window.confirm(Locale.LS("Adjust timestep for best audio quality and performance?"))) {
	...
	if (ct * sampleStep < .05) {
	    Window.alert(
		    Locale.LS("Audio data is not ready yet.  Increase simulation speed to make data ready sooner."));
	    return;
	}
